    ...
    const VastAPI = {
        ...
        createInstance: function (offerId, label, disk) {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        const voucher = response.voucher || 'Not provided';
                        ...
                        VastNotifications.success(
                            ...
                            {
                                duration: 5000,
                                action: `<button class="vast-toast-action" onclick="$('a[href=\\'#instances\\']').tab('show'); VastApp.currentTab = 'instances'; VastAPI.loadInstances();">
                    ...
                    if (response.success) {
                        ...
                        if (destroyed.length > 0) {
                            ...
                            VastNotifications.warning(
                                ...
                                {
                                    duration: 10000,
                                    action: `<button class="vast-toast-action" onclick="VastAPI.loadInstances()">
