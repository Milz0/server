...
(function () {
    ...
    window.VastModals.Bulk = {
        ...
        showRentModal: function () {
            ...
            const quirkyPrefix = VastUtils.generateQuirkyName();
            $('#bulk-label-prefix').val(quirkyPrefix).removeClass('is-invalid is-valid');
            ...
            offersList += '<thead style="position: sticky; top: 0; z-index: 1;">';
            offersList += '<tr>';
            offersList += '<th style="width: 40px;">#</th>';
            offersList += '<th>GPU</th>';
            offersList += '<th>Location</th>';
            offersList += '<th class="text-end" style="width: 100px;">Cost/hr</th>';
            offersList += '</tr>';
            offersList += '</thead>';
            offersList += '<tbody>';
            ...
            VastApp.selectedOffers.forEach(function (offer, index) {
                ...
                offersList += '<td class="text-end">';
                offersList += '<span class="badge bg-info">$' + hourlyCost.toFixed(3) + '/hr</span>';
                offersList += '</td>';
                offersList += '</tr>';
            });
        ...
        executeRent: function (disk, image) {
            ...
            statusHtml += '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
            ...
            function createNext(index) {
                if (index >= totalCount) {
                    ...
                    VastModals.Bulk.showRentSummary(successCount, failCount, results);
                    return;
                }

                const offer = VastApp.selectedOffers[index];
                // Generate a unique quirky name for each instance
                const label = VastUtils.generateQuirkyName();
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            $('#instance-status-' + index).html(
                                ...
                                '<strong>' + label + '</strong> - ' +
                                '<span class="text-danger">' + VastUtils.truncate(errorMsg, 200) + '</span>'
                            );
                        }
                    },
                    error: function (xhr) {
        ...
        showRentSummary: function (successCount, failCount, results) {
            const totalCount = successCount + failCount;

            let summaryHtml = '<div class="alert alert-' + (failCount === 0 ? 'secondary' : 'secondary') + ' alert-dismissible fade show" role="alert">';
            ...
            if (successCount > 0) {
                ...
                results.filter(r => r.success).forEach(function (result) {
                    ...
                    summaryHtml += '<td><span class="badge bg-secondary">#' + result.instanceId + '</span></td>';
            ...
            if (failCount > 0) {
                ...
                results.filter(r => !r.success).forEach(function (result) {
                    ...
                    summaryHtml += '<td><strong>' + result.label + '</strong></td>';
                    summaryHtml += '<td><small>' + result.gpu + '</small></td>';
                    summaryHtml += '<td><small class="text-danger">' + VastUtils.truncate(result.error, 200) + '</small></td>';
                    summaryHtml += '</tr>';
                });
            ...
            if (successCount > 0) {
                summaryHtml += '<div class="alert alert-dark mb-0 py-2">';
        ...
        init: function () {
            ...
            $('#confirmBulkRent').off('click').on('click', function () {
                ...
                VastModals.Bulk.executeRent(disk, image);
            });
