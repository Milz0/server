...
(function () {
    ...
    window.VastModals.Instance = {
        ...
        showCreateModal: function (offerId, offerData) {
            ...
            // Generate a random quirky name (hashing/cracking themed)
            const randomName = VastUtils.generateQuirkyName();
            $('#modal-label').val(randomName).removeClass('is-invalid is-valid');

            $('#modal-disk').val('25').removeClass('is-invalid is-valid');

            $('#modal-image').removeClass('is-invalid is-valid');
        ...
        showDetails: function (instance) {
            ...
            if (instance.agentStats) {
                ...
                if (stats.isActive === true) {
                    agentHtml += '<span class="badge bg-success"><i class="fas fa-check-circle"></i> Active</span>';
                } else {
                    agentHtml += '<span class="badge bg-secondary"><i class="fas fa-times-circle"></i> Inactive</span>';
                }
                agentHtml += '</div>';
        ...
        init: function () {
            ...
            // Trigger initial validation on modal open
            $('#createInstanceModal').on('show.bs.modal', function () {
                $('#modal-label').trigger('input');
                $('#modal-disk').trigger('input');
            });
        }
    };
