...
class VastUtils
{
  ...
  public static function getHashtopolisUrl()
  {
    ...
    // Fallback: automatic detection from current request (proxy-safe)
    $proto = null;
    if (!empty($_SERVER['HTTP_X_FORWARDED_PROTO'])) {
      $proto = $_SERVER['HTTP_X_FORWARDED_PROTO'];
    }
    if ($proto !== null) {
      $protocol = (stripos($proto, 'https') !== false) ? "https://" : "http://";
    } else {
   ...
   */
  private static function isVastDebugEnabled()
  {
    $v = getenv('VAST_DEBUG');
    if ($v === false) {
      return false;
  ...
  public static function callVastApi($endpoint, $method = 'GET', $data = null)
  {
    ...
    if ($debug) {
      ...
      if ($curlErr !== null) {
        error_log("[VastUtils] cURL error: " . $curlErr);
      }

      $responsePreview = ($response !== false && strlen($response) > 2000)
        ? substr($response, 0, 2000) . '...[TRUNCATED]'
        : $response;

      if (is_string($responsePreview)) {
    ...
    if ($response === false) {
      throw new HTException("Vast API request failed (cURL): " . ($curlErr ?? 'unknown error'));
    }

    $decoded = json_decode($response, true);
    if ($decoded === null && json_last_error() !== JSON_ERROR_NONE) {
      $preview = strlen($response) > 300 ? substr($response, 0, 300) . '...[TRUNCATED]' : $response;
      throw new HTException("Vast API returned non-JSON response (HTTP $httpCode): " . $preview);
    }

    if ($debug && isset($decoded['offers']) && is_array($decoded['offers'])) {
  ...
  /**
   * Build default filters for searching GPU offers (for /bundles/)
   * @param array $overrides Optional filter overrides from request
   * @return array Filters array compatible with Vast /bundles/
   */
  public static function buildOfferFilters($overrides = [])
  {
    ...
    // Type: on-demand
    if (isset($overrides['type']) && $overrides['type'] !== '' && $overrides['type'] !== null) {
      $filters['type'] = $overrides['type'];
      unset($overrides['type']);
    } else {
      // preserve your old default
      $filters['type'] = 'on-demand';
    }

    if (isset($overrides['reliability']) && $overrides['reliability'] !== '' && $overrides['reliability'] !== null) {
      $rel = floatval($overrides['reliability']);
      ...
      $filters['reliability'] = ['gte' => $rel];
      unset($overrides['reliability']);
    }

    // UI min_gpu uses gpu_num -> Vast uses num_gpus
    if (isset($overrides['gpu_num']) && $overrides['gpu_num'] !== '' && $overrides['gpu_num'] !== null) {
      $filters['num_gpus'] = ['gte' => intval($overrides['gpu_num'])];
      unset($overrides['gpu_num']);
    }

    // Max $/hr mapping: UI uses totalHour -> Vast uses dph_total
    if (isset($overrides['totalHour']) && $overrides['totalHour'] !== '' && $overrides['totalHour'] !== null) {
      $filters['dph_total'] = ['lte' => floatval($overrides['totalHour'])];
      unset($overrides['totalHour']);
    }
    ...
    foreach ($uiOnlyKeys as $k) {
      if (isset($overrides[$k])) {
        unset($overrides[$k]);
      }
    }

    // Merge any remaining overrides at top level (best-effort)
  ...
  /**
   * Search for GPU offers on Vast.ai (uses /bundles/ to support high limits)
  ...
  {
    $payload = self::buildOfferFilters($filters);

    if (self::isVastDebugEnabled()) {
      error_log("[VastUtils] Searching offers via /bundles/ with payload: " . json_encode($payload));
    }

    $result = self::callVastApi('/bundles/', 'POST', $payload);
  ...
  public static function destroyInstance($instanceId)
  {
    ...
    // Destroy the Vast instance first (avoid orphaning remote instances if local cleanup fails)
    $result = self::callVastApi('/instances/' . $instanceId . '/', 'DELETE');

    // Clean up agent and voucher after successful destroy
    try {
      self::cleanupVastInstance($instanceId);
    } catch (Exception $e) {
      if (self::isVastDebugEnabled()) {
        error_log("[VastUtils] Agent cleanup failed after destroy: " . $e->getMessage());
      }
    }

    return [
      'success' => true,
      'message' => ($result['msg'] ?? 'Instance destroyed successfully')
    ];
  }
